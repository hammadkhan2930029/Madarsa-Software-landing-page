        ...
        <div className="mx-auto max-w-7xl px-4 py-10 text-right sm:px-6 lg:px-8">
          <div className="page-reveal max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-theme-body text-slate-600 dark:text-slate-300">
              اپنی مدرسہ یا ادارے کی تفصیلات بھیجیں۔ ہماری ٹیم آپ کو ڈیمو، قیمت اور سیٹ اپ کے بارے میں رہنمائی دے گی۔
            </p>
          </div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {cards.map((card, index) => (
              <article
                ...
                className="contact-info-card soft-panel rounded-lg border border-themeBorder bg-themeSurface/92 p-5 shadow-card-theme backdrop-blur"
                style={{ animationDelay: `${index * 90}ms` }}
              >
                <p className="text-theme-kicker font-black text-themePrimary">{card.label}</p>
                <h2 className="mt-3 break-words text-theme-body font-bold text-themeText" dir={card.direction ?? 'rtl'}>
                  {card.value}
                </h2>
          ...
          <div className="contact-info-card soft-panel mt-4 rounded-lg border border-themeBorder bg-themeSurface/92 p-5 text-right shadow-card-theme backdrop-blur">
            <p className="text-theme-kicker font-black text-themePrimary">دفتر کا پتہ</p>
            <h2 className="mt-3 text-theme-body font-bold text-themeText" dir="ltr">
              {address}
            </h2>
          </div>
        </div>
