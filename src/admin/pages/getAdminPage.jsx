...
export function getAdminPage(path, openEditor, data = {}, actions = {}) {
  ...
  if (path === '/admin/slider') return <SliderPage openEditor={openEditor} sliderModules={data.sliderModules} onDelete={(row) => actions.deleteRecord?.('slider-modules', row)} onRestore={(row) => actions.restoreRecord?.('slider-modules', row)} />
